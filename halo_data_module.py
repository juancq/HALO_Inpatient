...
class EHRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ehr[0,0, self.config.start_token_idx] = 1  # Set the first visits to be the start token
...
class HALODataModule(L.LightningDataModule):
    def __init__(self, config, batch_size, train_name, val_name, train_split=1, shuffle=True):
        super().__init__()
        self.config = config
        ...
        # include test set here


    def setup(self, stage=None):
        df_trn = pl.scan_parquet(self.train_name)
        train_ehr_dataset = df_trn.collect(streaming=True).sample(fraction=self.train_split, seed=1)
        val_ehr_dataset = pl.read_parquet(self.val_name)
        
        # used for getting embeddings of a patient trajectory
        if stage == 'embeddings':
            include_id = True
            self.shuffle = False
        else:
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, 
                shuffle=self.shuffle, num_workers=self.config.num_workers, collate_fn=collate_fn)


    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.batch_size, 
                shuffle=False, num_workers=self.config.num_workers, collate_fn=collate_fn)
